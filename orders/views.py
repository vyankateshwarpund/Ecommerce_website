    ...
    try:
        ...
        if address_id:
            ...
            if addr:
                ...
                shipping_phone = getattr(getattr(request.user, 'profile', None), 'phone', '') or '9876543210'
                shipping_address = 'Default Delivery Address'
                shipping_city = 'Mumbai'
                shipping_state = 'Maharashtra'
                shipping_pincode = '400001'
        else:
            shipping_name = request.POST.get('full_name') or request.user.get_full_name() or request.user.username
            shipping_phone = request.POST.get('phone') or getattr(getattr(request.user, 'profile', None), 'phone', '') or '9876543210'
            shipping_address = request.POST.get('street_address') or request.POST.get('address_line_1') or request.POST.get('address', '') or 'Standard Delivery Address'
            shipping_city = request.POST.get('city') or 'Mumbai'
            shipping_state = request.POST.get('state') or 'Maharashtra'
            shipping_pincode = request.POST.get('pincode') or '400001'

            if request.POST.get('full_name') and request.POST.get('street_address'):
                try:
                    Address.objects.create(
                        user=request.user,
        ...
        traceback.print_exc()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'status': 'error', 'message': f'Order error: {str(e)}'}, status=400)
        messages.error(request, f'Unable to place order: {str(e)}')
        return redirect('orders:checkout')
